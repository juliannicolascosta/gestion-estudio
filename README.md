...
- Las descargas solicitadas mientras otra está activa se incorporan a una cola oculta y se ejecutan en orden, conservando el expediente de origen. Cada movimiento muestra si está en cola, descargando o falló; los errores pueden reintentarse sin detener los pedidos siguientes.
...
- El motor probado de **Extractor SISFE** forma parte del paquete `gestor_documental`: la generación de cédulas no busca otra aplicación, ejecutable ni carpeta externa. El instalador incluye el código, las reglas, PyMuPDF y el catálogo público necesarios.
...
La versión estable actual es la `0.13.7`, una evolución del MVP `delivery-sisfe`. Incorpora un centro único de configuración para profesionales, modelos y firmador, incorporación segura de carpetas externas como casos mediante copia y vista previa, firma PAdES con sesión de token reutilizable y apariencia visible opcional, ficha ampliada por tipo de caso, generación contextual desde modelos Word, semáforo del directorio, interpretación inicial de SISFE, cola de descargas, clientes vinculados con varios casos y respaldo verificable de cada Ubicación. Conserva las varias **Ubicaciones del Estudio**, la búsqueda conjunta y Xólido como alternativa. Una carpeta compartida de Google Drive debe aparecer en el Explorador mediante Google Drive para escritorio.
